...
def parse_name_with_suffix(full_name, default_last_name=''):
    ...
    # Remove parentheses and their contents (including standalone closing parens)
    full_name = re.sub(r'\([^)]*\)', '', full_name)
    full_name = re.sub(r'\)', '', full_name)  # Remove any standalone closing parens
...
# 1. Load base data (ceremony files + faculty marshals)
print("Loading base Culbreth data (ceremony files + faculty marshals)...")
...
print(f"  Loaded {len(culbreth_records)} base records")

# 2. Process MASTER file - for BOTH locations
print("\nProcessing MASTER file for both locations...")
master = pd.read_excel('C:/Users/Ben/Downloads/Finals Weekend Parking List (MASTER) 2026.xlsx',
                       sheet_name='CH&MH, Culbreth, JPJ. Sat & Sun')

master_culbreth_count = 0
master_carrs_count = 0

for idx, row in master.iterrows():
    first_name = fix_capitalization(row.get('First_Name', ''))
    last_name = fix_capitalization(row.get('Last_Name', ''))
    affiliation_raw = row.get('Affliation', '')
    day = row.get('Day', '')
    ...
    if culbreth_passes > 0:
        ...
        master_culbreth_count += 1

    # Check CARR'S HILL parking
    carrs_col = "Carr's Hill / Madison Hall"
    carrs_passes = row.get(carrs_col, 0)
    try:
        carrs_passes = float(carrs_passes) if pd.notna(carrs_passes) else 0
    except:
        carrs_passes = 0
    ...
    if carrs_passes > 0:
        carrs_hill_records.append({
            ...
            'Affiliation': affiliation,
            'Ceremony Day': ceremony_day,
            'Number of Passes': int(carrs_passes),
            'Notes': ''
        })
        master_carrs_count += 1

print(f"  Added {master_culbreth_count} to Culbreth from MASTER")
print(f"  Added {master_carrs_count} to Carr's Hill from MASTER")

# 3. Add BOV (to Culbreth only based on previous logic)
print("\nAdding BOV to Culbreth...")
...
for idx, row in bov_df.iterrows():
    ...
    for guest_name in guest_names:
        ...
        # Skip day-related words and ceremony references
        if any(x in guest_lower for x in ['saturday', 'sunday', 'valedictory', 'sat,', 'sun,']):
            continue

        first, last, used_default = parse_name_with_suffix(guest_name, default_last_name=last_name)

        # Skip if first name is just a day abbreviation
        if first and first.lower() in ['sat', 'sun', 'mon', 'tue', 'wed', 'thu', 'fri']:
            continue
...
# 4. Add Credentials (to Culbreth only - only has Culbreth column)
print("\nAdding Credentials Master List to Culbreth...")
...
# 5. Process Platform & Procession for Carr's Hill
print("\nProcessing Platform & Procession Party for Carr's Hill...")
...
for idx, row in platform_df.iterrows():
    ...
    if both == 'X':
        ...
        total_passes = max(sat_passes, sun_passes, 1)
...
print(f"  Added {platform_count} Platform Party records")
...
# Remove standalone Jr/Sr entries
...
print(f"  Carr's Hill / Madison Hall: {len(df_carrs)}")

# Verify Beardsleys are in both
beardsley_culbreth = df_culbreth[df_culbreth['Last Name'].str.contains('Beardsley', case=False)]
beardsley_carrs = df_carrs[df_carrs['Last Name'].str.contains('Beardsley', case=False)]
print(f"\nBeardsley verification:")
print(f"  In Culbreth: {len(beardsley_culbreth)}")
print(f"  In Carr's Hill: {len(beardsley_carrs)}")
